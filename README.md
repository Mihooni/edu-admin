...
| 📱 小程序 | 微信开发者工具导入 `miniprogram/` | 开发版自动连本机 :3001 |

**体验账号**（示例数据自动生成）：

| 身份 | 手机号 | 密码 | 入口 |
|---|---|---|---|
| 管理员 | `13800000001` | `123456` | Web 管理后台 + 小程序管理端 |
| 教练 | `13800000011`（王教练）等 | `123456` | 小程序课表 / 签到 / 点评 |
| 家长 | `13900000001`（小明爸爸）等 | 无需密码 | 小程序查看训练 / 报名 / 通知 |
...
npm run verify        # 扩展验收套件（26 套件，需服务运行中，见 TEST-GUIDE.md）
```
...
正式上线（公网服务器 + HTTPS 域名 + 小程序发布 + 数据备份）完整步骤见 **[`部署上线说明.md`](部署上线说明.md)**。
...
### 家长端小程序
- 会员身份卡（时效制 / 次数制）、通知中心、活动报名、积分明细、订单、请假申请、成长记录（教练课后点评）、反馈（支持机构回复闭环）

### 教练端小程序
- 今日课表、点名签到、请假审批、发布通知、课后点评
...
服务器 7×24 运行推荐用 pm2 守护：`npm i -g pm2 && cd backend && pm2 start server.js --name starclass && pm2 save`。完整公网部署（域名 / HTTPS / 小程序发布）见 [`部署上线说明.md`](部署上线说明.md)。
...
| 小程序怎么体验？ | 微信开发者工具导入 `miniprogram/` 目录（AppID 用测试号即可），开发版自动连本机 :3001 |
| 不配微信小程序能用吗？ | 能。手机号 + 密码登录全功能可用；配置 `WX_APPID`/`WX_SECRET` 后解锁微信一键登录与订阅消息 |
